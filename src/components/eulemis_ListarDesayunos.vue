  ...
  <div>
    ...
    <main>
       <b-container fluid style="margin-top: -100px">
        <b-row>
          <b-col
            v-for="(cat, index) in desayunoCategories"
            :key="index"
            cols="12"
          >
            <h2 v-show="titlefood" class="text-capitalize">{{ cat.title }}</h2>
            <v-row>
              <v-col
...
export default {
  ...
  methods: {
    ...
    async getCategories(){
      this.isLoading = true;
      let response = await axios.get("http://lidotel.test/api/auth/food_category" + '/' + this.hotel_id + '/' + this.foodID + '/desayuno', {
            headers: {
    ...
    incrementItemQt(food) {
			
			  food.quantity++
        //this.getQuantity(food)
        console.log(food )
		},
		decrementItemQt(food) {
