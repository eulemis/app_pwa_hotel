...
export default new Vuex.Store({
	...
	mutations: {
		...
		setFood(state, data) {
			...
			//state.typefood = data.map(cat => {
			state.typefood = data.map(food => {
					 food.quantity = 0,
					 food.comment = '',
				 ...
				 //})
			//return typefood
			/* state.typefood = foodItems.map(typefood => {
				typefood.quantity = 0,
					typefood.comment = '',
					typefood.user_id = this.user_id
				return typefood
			}) */
		},
		setFoodPagination({ state, data }) {
		...
		typefood(state, typefood) {
			console.log(state, typefood)
			state.typefood = typefood
		},
		
		setFoodComment(state, pack) {
			let index = state.typefood.findIndex(({ id }) => id == pack[0])
			state.typefood[index].comment = pack[1]
		},
		incrementItemFood(state, idFood) {
			console.log(state.typefood, idFood, state.typefood, state.typefood.findIndex(({ id }) => id == idFood))
			
			let index = state.typefood.findIndex(({ id }) => id == idFood)
			state.typefood[index].quantity++
		},
		decrementItemFood(state, idFood) {
			let index = state.typefood.findIndex(({ id }) => id == idFood)
			if (state.typefood[index].quantity > 0)
				state.typefood[index].quantity--
		},
		resetItemSelection(state, idFood) {
			let index = state.typefood.findIndex(({ id }) => id == idFood)
			state.typefood[index].quantity = 0
		},
	...
	getters: {
		...
		getFoodItems({ typefood }) {
			return typefood
		},
		getSelectedFood({ typefood }) {
			//console.log("foodPagination", foodPagination, foodPagination.data, foodPagination.data.filter(({ quantity }) => quantity > 0 )
			return typefood.filter(({ quantity }) => quantity > 0)
		},
		getHotelId({ typefood }) {
			...
			return new Promise((resolve, reject) => {
				axios.get(`${ENV.BASE_URL}/food_category/${getters.getHotelId}/${food}/${category}`, {
					...
					.then(response => {
						if (response.status == 200) {
							if (response.data) {
								const typefood = response.data

								commit('setFood', typefood)
								resolve(response.data)
